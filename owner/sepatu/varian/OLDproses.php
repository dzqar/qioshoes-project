...
// $idDataSepatu = $_GET['id_data_sepatu'];
...
if (isset($_POST['btn']) || isset($_GET['btn'])) {
	if ($_POST['btn'] === 'TambahVarianSepatu') {
		...
		if ($countNS <= 0) {
			...
			// Cek Varian Warna
			$dataWarna = mysqli_query($db->koneksi,"SELECT warna,gambar_sepatu FROM data_sepatu WHERE id_sepatu='$idSepatu' AND warna LIKE'%$warna%' AND ukuran='$ukuranSepatu'");
